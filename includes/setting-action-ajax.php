...
class ctt_load_ajax extends ctt {
	public function __construct() {
		parent::__construct();
		add_action('wp_ajax_ctt_tpl_settings', [$this, 'ctt_box_settings']);
		add_action('wp_ajax_ctt_auth_settings', [$this, 'ctt_auth_box_settings']);
		add_action('wp_ajax_ctt_image_settings', [$this, 'ctt_image_settings']);
	...
	public function ctt_box_settings() {
		...
		if ($tpl <= 12) {
			if ($tpl < 6) {
				$_box['box_' . $tpl] = ['callforaction' => $dary['cta-txt'],
	...
	public function ctt_hint_box_settings() {
		...
		$val = [];
		parse_str($data, $val);
		$set_option = ['background' => $val['hbox-type'], 'color' => $val['hbox-color']];
	...
	public function ctt_tweet_settings() {
		...
		$short_url = $val['short-url'] ?? 0;
		$set_option = ['url' => $short_url, 'ctt-handler' => $val['twt-handler']];
